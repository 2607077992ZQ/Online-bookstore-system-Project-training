...
namespace BookShop.BLL
{
	...
	public partial class Users
	{
		...
		/// <returns></returns>
		public Model.Users SelectName(string name, string pwd)
		{
			List<Model.Users> modList = GetModelList("");
			if(pwd=="")
            {
				Model.Users mod = modList.Find(p => p.LoginId == name);
				return mod;
			}
			else
            {
				Model.Users mod = modList.Find(p => p.LoginId == name && p.LoginPwd==pwd);
				return mod;
			}
